...
SCHEMA_VERSION = "scaffoldseal-d0-full-plan-v2"
...
CORRECTED_PROJECTED_GPU_HOURS = 49.36635972118464
MAX_PROJECTED_GPU_HOURS = 72.0
...
FROZEN_PROJECT_INPUTS = (
    ...
    "src/d0_fold_scoped_governance.py",
    "src/r1c0_dmpnn_pilot.py",
    "src/split_safe.py",
    "artifacts/v2_r0/outer_record_assignments.csv",
    "artifacts/v2_r0/outer_fold_manifest.csv",
    "artifacts/v2_r0/inner_basket_manifest.csv",
    "artifacts/v2_r0/pre_fit_contract_manifest.csv",
    "artifacts/v2_r0/fit_boundary_policy.json",
    "artifacts/v2_r0/SHA256SUMS",
    "artifacts/v2_r0/d0_full_runner_repair1/fold_scoped_target_manifest.json",
    "artifacts/v2_r0/d0_full_runner_repair1/label_free_features.csv",
    *(f"artifacts/v2_r0/d0_full_runner_repair1/outer_{fold:02d}_training_targets.csv" for fold in OUTER_FOLDS),
)
DEFAULT_MANIFEST = "d0_full_run_manifest.json"
DEFAULT_ACCEPTANCE = "d0_full_run_acceptance.json"
FOLD_VIEW_ROOT = "artifacts/v2_r0/d0_full_runner_repair1"
...
def scientific_runner_record(project_root: Path) -> dict[str, object]:
    path = (project_root / "src/d0_full_runner.py").resolve()
    payload = _scientific_runner_bytes(path)
    return {
        "relative_path": "src/d0_full_runner.py",
...
def _assert_baseline_source_identity(baseline_root: Path) -> list[dict[str, object]]:
    commit = _git_output(["git", "-C", str(baseline_root), "rev-parse", "HEAD"])
...
def _stage_namespace(kind: str, suffix: str) -> dict[str, str]:
    base = f"{kind}/{suffix}"
    return {
        "work_attempt_template": f"runs/r1c0_d0_full_v1/{base}/attempt_{{attempt_number:04d}}",
        "output_attempt_template": (
            f"artifacts/r1c0_d0_full_v1/{base}/attempt_{{attempt_number:04d}}"
        ),
...
def build_full_plan(project_root: Path, baseline_root: Path) -> dict[str, object]:
    ...
    inner = pd.read_csv(project_root / "artifacts/v2_r0/inner_basket_manifest.csv")
    contracts = pd.read_csv(project_root / "artifacts/v2_r0/pre_fit_contract_manifest.csv")
    ...
    if len(features) != 6895 or len(outer) != 6895 or len(contracts) != 72:
    ...
    if governance.get("schema_version") != "scaffoldseal-d0-fold-scoped-targets-v1":
    ...
    scientific_lock = {
        ...
        "metric_only_label_source": {
            ...
            "access_role": "OPEN_ONLY_AFTER_ALL_90_PREDICTIONS_VALIDATE",
        },
    ...
    block_to_basket = inner.set_index(["outer_fold", "sealed_block_id"])["inner_basket"]
    for fold in OUTER_FOLDS:
        ...
        fold_rows["inner_basket"] = [
            int(block_to_basket.loc[(fold, block)]) for block in fold_rows["sealed_block_id"]
        ]
    ...
    metric_stage = _stage(
        ...
        key={"model": "D0", "split": "joint_block_lobo"},
        ...
        namespace=_stage_namespace("metrics", "joint_block_lobo"),
        execution={
            "gpu": False,
            "model_construction": False,
    ...
    counts = {
        ...
        "pampa_inner_fits": 72,
        "stopping_epoch_selections": 18,
        "pampa_outer_fits": 90,
        "pampa_outer_predictions": 90,
        "sealed_oof_metric_stages": 1,
        "scientific_fits_total": 167,
    ...
    plan = {
        ...
        "scheduler": {
            ...
            "ledger_relative_path": "artifacts/r1c0_d0_full_ledger_v1",
        },
        "counts": counts,
        "stages": stages,
    }
...
def validate_plan(plan: Mapping[str, object]) -> None:
    ...
    for stage in stages:
        ...
        if kind == "delaney_pretraining":
            ...
            if dict(key) != {"model": "D0", "split": "joint_block_lobo"}:
                raise ValueError("Metric key invariant changed")
            expected_namespace = _stage_namespace("metrics", "joint_block_lobo")
    ...
    expected_counts = {
        ...
        "sealed_oof_metric_stages": 1,
        "scientific_fits_total": 167,
        "manifest_stage_records_total": 186,
...
def load_external_acceptance(path: Path) -> dict[str, object]:
    ...
    if acceptance.get("schema_version") != "scaffoldseal-d0-external-acceptance-v1":
...
def verify_candidate_anchor(
    ...
    *,
    ...
    current_runner = _project_file_record(project_root, "src/d0_full_runner.py")
...
class D0FullExecutor:
    ...
    def _check_gpu_budget(self) -> None:
        ...
        if not math.isfinite(total) or total >= MAX_PROJECTED_GPU_HOURS * 3600.0:
            raise RuntimeError("Cumulative D0 GPU runtime reached the locked 72-hour limit")
    ...
    def _cleanup_accepted_work(self, stage: Mapping[str, object]) -> None:
        ...
        allowed_root = (self.project_root / "runs/r1c0_d0_full_v1").resolve()
        try:
    ...
    def _load_fold_scoped_frame_and_contracts(self, outer_fold: int):
        from split_safe import contracts_from_manifests
        ...
        outer = pd.read_csv(self.project_root / "artifacts/v2_r0/outer_record_assignments.csv")
        inner = pd.read_csv(self.project_root / "artifacts/v2_r0/inner_basket_manifest.csv")
        ...
        contract_records = features.rename(columns={"SMILES": "canonical_smiles"})
        return features, frame, contracts_from_manifests(contract_records, outer, inner)
        ...
        try:
            ...
            expected_slots = {
                ...
                for row in pd.read_csv(
                    self.project_root / "artifacts/v2_r0/outer_record_assignments.csv"
